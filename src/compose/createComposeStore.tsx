...
export default function (props) {
  return {
    ...
    hydrateFromEntity() {
      ...
      console.log('ENTITY TYPE IS ', entity.custom_type);
      ...
      if (entity.custom_type === 'batch') {
        ...
        console.log('calling addAttachment!');
        this.attachments
          .addAttachment(false)
    ...
    setTimeCreated(time?: number | null) {
      ...
      console.log('set time created to ', this.time_created);
    },
    toggleNsfw(opt) {
      if (opt === 0) {
        this.nsfw = [];
    ...
    async selectFromGallery(mode) {
      console.log('HI.', this.attachments);
    ...
    async onMediaFromGallery(media: PickedMedia | PickedMedia[]) {
      ...
      if (Array.isArray(media)) {
        ...
        console.log('Here is reached. Attachments ', this.attachments);
        this.attachments.attachMedia(
          {
            ...media,
            type: media.mime,
        ...
        console.log('I think we got to this line.');
        this.mode = 'text';
      }
    },
    /**
    ...
    async submit() {
      ...
      try {
        ...
        console.log(
          'this.entity?.guid is ',
          this.entity?.guid,
          typeof this.entity?.guid,
          this.isEdit,
        );

        // keep the container if it is an edited activity
        if (this.isEdit && typeof this.entity?.guid !== 'undefined') {
          newPost.container_guid = this.entity.guid;
          console.log('got here.');
        } else {
          newPost.container_guid = uuid();
        }
        ...
        console.log('tc is', this.time_created);
        const obj = {
          guid: newPost.container_guid,
          user_id: uuid(),
          message: newPost.message,
          attachments: newPost.attachment_guids,
        };
        if (!this.isEdit) {
          obj['time_completed'] = new Date(this.time_created).toISOString();
        }
        ...
        console.log('compose item is ', data, error);
        ...
        if (this.isEdit) {
          ...
          const newEntity = ActivityModel.create(data);
          // this.entity!.update(data); // fixme tgorer why does it say update() func undefined?
          newEntity!.setEdited('1');
        ...
        return ActivityModel.create(data); //fixme tgorer is this really valid? IMPORTANT.
      } catch (e) {
