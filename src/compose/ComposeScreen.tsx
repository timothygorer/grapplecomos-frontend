...
const ComposeScreen: React.FC<ScreenProps> = props => {
  ...
  const onPressPost = useCallback(async () => {
    ...
    console.log('submitted entity is ', entity);
  ...
  console.log('store.time_created is ', store.time_created);
  ...
  const rightButton = store.isEdit ? (
    ...
    <IconButtonNext
      ...
      scale
      onPress={onPressPost}
      disabled={!store.isValid && !store.time_created}
      color={store.isValid && store.time_created ? 'Link' : 'Icon'}
