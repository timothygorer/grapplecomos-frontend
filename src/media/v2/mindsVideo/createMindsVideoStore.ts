...
const createMindsVideoStore = ({
  ...
  const store = {
    ...
    setVideo(video: AVPlaybackSourceObject | null) {
      ...
      if (video) {
        this.player?.loadAsync(video).then(_ => {
          console.log('loaded async video.');
    ...
    setPositionAsync(startPositionInMillis) {
      this.player?.setPositionAsync(startPositionInMillis);
      console.log('ready for display is set to true.');
    ...
    _init(): Cancelable<void> {
      return new Cancelable(async (resolve, reject) => {
        if ((!this.sources || this.sources.length === 0) && this.entity) {
          try {
            console.log('at least got to this point.');
            ...
            const videoObj = {
              sources: [
                {type: 'video/mp4', src: this.entity.custom_data[0].src},
              ],
            };
            console.log('videoObj is ', videoObj);
