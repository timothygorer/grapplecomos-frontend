...
export default class CommentsStore {
  ...
  getParentPath() {
    console.log(
      ...
      this.parent,
      'and parent path is',
      this.parent && this.parent.child_path ? this.parent.child_path : '0:0:0',
    );
  ...
  getFocusedCommentUrn() {
    console.log('got to cmmnts store');
  ...
  setComments(response, descending) {
    ...
    console.log('setComments response:', response);

    if (response.comments) {
      const comments = CommentModel.createMany(response.comments);
      console.log('created commments:', comments);
      console.log('comments[0] is ', comments[0]);
      ...
      if (descending) {
        ...
        comments.forEach(c => {
          ...
          console.log('this.commments after unshift', this.comments);
        });
      }
    }
    this.reversed = false;
  ...
  post = async () => {
    ...
    if (this.edit) {
      console.log('this.edit is ', this.edit);
    ...
    try {
      ...
      console.log('pushing comment:', data);
  ...
  async updateComment(comment: any) {
    ...
    try {
      ...
      // } else {
      console.log('setting desc to ', this.text);
      // }
      this.setText('');
      this.edit = undefined;
      this.setShowInput(false);
  ...
  setCommentDescription(comment, description) {
    console.log('comment is ', comment, ' and desc is ', description);
