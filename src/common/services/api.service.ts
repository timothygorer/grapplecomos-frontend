...
export class ApiService {
    ...
    return Object.keys(params)
      .map(k => {
        if (params[k] instanceof ParamsArray) {
          ...
          // For complex keys like 'subtile.chapter.name', encode only the value
  ...
  async get<T>(
    ...
    console.log('response is ', this.buildUrl(url, params));
  ...
  async post<T>(
    ...
    const headers = {
      apiKey:
        'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imx5ZXdsaHhxZmlxZ21idmVvaWdsIiwicm9sZSI6ImFub24iLCJpYXQiOjE2OTI3NTE1MDIsImV4cCI6MjAwODMyNzUwMn0.ZPchJBdruN-Avt_sNLIQ6ktolY6vPOD_OuQ4Wp5c2rE',
      'Content-Type': 'application/json',
    };

    const response = await this.axios.post(
      'https://lyewlhxqfiqgmbveoigl.supabase.co/graphql/v1',
      {
        query:
          'query FetchNewsfeed($limit: Int!, $cursor: String, $inFeedNoticesDelivered: [String!]) { entities_activityCollection(first: $limit) { edges { cursor ... on ActivityEdge { explicitVotes } node { guid user_id thumbs_up_count thumbs_up_user_guids thumbs_down_user_guids __typename ... on ActivityNode { legacy } } } } }',
      },
      {headers},
    );
