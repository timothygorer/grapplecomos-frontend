...
export class EntitiesStorage {
  ...
  cleanEntity(entity: storageEntity) {
    if (
      entity['thumbs_up_user_guids'] &&
      Array.isArray(entity['thumbs_up_user_guids'])
    ) {
      entity['thumbs_up_user_guids'] = entity['thumbs_up_user_guids'].filter(
        (guid: string): boolean => guid === sessionService.guid,
      );
    }

    if (
      entity['thumbs_down_user_guids'] &&
      Array.isArray(entity['thumbs_down_user_guids'])
    ) {
      entity['thumbs_down_user_guids'] = entity[
