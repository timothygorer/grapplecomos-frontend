...
export default observer(function InteractionsBar({
  ...
  return (
    <View style={containerStyle}>
      {entity['thumbs_up_count'] > 0 && (
        <PressableScale
          innerStyle={buttonInnerStyle}
          style={buttonStyle}
          onPress={onShowUpVotesPress}>
          <Counter count={entity['thumbs_up_count']} style={countStyle} />
          <MText style={textStyle}>
            {' '}
            {i18n.t('interactions.upVotes', {
              count: entity['thumbs_up_count'],
            })}
          </MText>
        </PressableScale>
      )}
      {entity['thumbs_down_count'] > 0 && (
        <PressableScale
          innerStyle={buttonInnerStyle}
          style={buttonStyle}
          onPress={onShowDownVotesPress}>
          <Counter count={entity['thumbs_down_count']} style={countStyle} />
          <MText style={textStyle}>
            {' '}
            {i18n.t('interactions.downVotes', {
              count: entity['thumbs_down_count'],
            })}
          </MText>
        </PressableScale>
      )}
