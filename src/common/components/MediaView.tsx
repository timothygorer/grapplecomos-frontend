...
export default class MediaView extends Component<PropsType> {
  ...
  showMedia() {
    ...
    console.log(
      'showMedia() called w/type',
      type,
      'this.props.entity?.custom_data?.length is ',
      this.props.entity?.custom_data?.length,
    );
    if (
      !type &&
  ...
  setPositionAsync(startPositionInMillis) {
    if (this.videoPlayer) {
      this.videoPlayer.setPositionAsync(startPositionInMillis);
    }
  }
