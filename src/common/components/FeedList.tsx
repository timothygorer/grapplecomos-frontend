...
> {
  ...
  onViewableItemsChanged = (change: {
    ...
    change.viewableItems.forEach(
      (item: {item: any}) => item?.item?.sendViewed?.(),
    );
  ...
  refresh = async () => {
    if (this.props.onRefresh) {
      ...
      console.log('feedlist refresh called.');
    ...
    return (
      <View testID={this.props.testID} style={containerStyle}>
        <AnimatedFlashList
          ...
          onEndReachedThreshold={0.0001} // 5 times the visible list height
