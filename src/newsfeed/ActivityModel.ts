...
import {MINDS_CDN_URI, MINDS_URI} from '../config/Config';
...
export default class ActivityModel extends BaseModel {
  ...
  'thumbs_down_user_guids': Array<string>;
  'thumbs_up_user_guids': Array<string>;
  seen?: boolean;
  rowKey?: string;
  boosted_guid?: string;
   ...
   * @param {string} size
   */
  getThumbSource(size = 'medium'): ImageSource {
  ...
  isGif(): boolean {
    console.log("GIF is", Boolean(this.custom_data && this.custom_data[0] && this.custom_data[0].gif))
  ...
  private onMetricsUpdate(event: string) {
    ...
    try {
      ...
      runInAction(() => {
        if (typeof metricsEvent['thumbs_up_count'] === 'number') {
          this['thumbs_up_count'] = metricsEvent['thumbs_up_count'];
        }
        if (typeof metricsEvent['thumbs_down_count'] === 'number') {
          this['thumbs_down_count'] = metricsEvent['thumbs_down_count'];
        }
      });
    } catch (e) {
...
decorate(ActivityModel, {
  //@ts-ignore
  'thumbs_down_count': observable,
  thumbs_up_count: observable,
  'comments:count': observable,
  thumbs_down_user_guids: observable,
  thumbs_up_user_guids: observable,
});
