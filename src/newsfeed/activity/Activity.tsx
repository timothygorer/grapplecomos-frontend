...
})) // fixme tgorer not sure what this does, or undoable
@observer
export default class Activity extends Component<ActivityProps> {
  ...
  navToActivity = async () => {
    ...
    if (this.props.isDiscoveryEntity) {
      const {data, error} = await supabase.from('tile').select('*,subtile(*,chapter(*))').eq('move_title', this.props.entity.move_title).single()
      if (data) {
         navOpts = {
          entity: {...this.props.entity, ...data},
          hydrate: false,
        };
      }
    } else {
    ...
    console.log('navving to activity.', navOpts, this.props.navigation);
