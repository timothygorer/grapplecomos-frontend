...
export const isAndroidTablet = () => {
  ...
  // Define a threshold to determine if the device is a tablet
  const smallestSide = Math.min(width, height);
  const tabletThreshold = 600; // Typically, devices with screen width greater than 600dp are considered tablets

  return smallestSide >= tabletThreshold;
};
